...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "ai_preferences", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column("id", sa.UUID(), nullable=False),
        sa.Column("created_at", sa.DateTime(), nullable=False),
        sa.Column("updated_at", sa.DateTime(), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("ACTIVE", "SUSPENDED", "ARCHIVED", "TRIAL", name="workspacestatus"),
            nullable=False,
        ),
        ...
        sa.ForeignKeyConstraint(
            ["plan_id"],
            ["plans.id"],
            name=op.f("fk_workspaces_plan_id_plans"),
            ondelete="SET NULL",
        ),
        sa.ForeignKeyConstraint(
            ["updated_by"],
            ["users.id"],
            name=op.f("fk_workspaces_updated_by_users"),
            ondelete="SET NULL",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_workspaces")),
    )
    op.create_index(
        op.f("ix_workspaces_country"), "workspaces", ["country"], unique=False
    )
    op.create_index(
    ...
    op.create_index(
        op.f("ix_workspaces_status"), "workspaces", ["status"], unique=False
    )
...
def downgrade() -> None:
    op.drop_constraint(
        op.f("fk_users_default_workspace_id_workspaces"), "users", type_="foreignkey"
    )
