...
async def setup_invites_env(async_db: AsyncSession, async_client: AsyncClient):
    ...
    viewer_role = Role(name=f"Viewer_{suffix}", role_type=RoleType.SYSTEM, is_system=True)
    ...
    for action in ["create", "read", "delete"]:
        result = await async_db.execute(select(Permission).filter_by(resource="workspace_invite", action=action))
        p = result.scalar_one_or_none()
        if not p:
            p = Permission(name=f"workspace_invite.{action}", resource="workspace_invite", action=action)
            async_db.add(p)
            try:
    ...
    ws = Workspace(name="Invite Test WS", slug=f"invite-ws-{suffix}", owner_id=owner.id, status=WorkspaceStatus.ACTIVE)
    async_db.add(ws)
    await async_db.commit()
...
async def test_revoke_invite(async_client: AsyncClient, setup_invites_env):
    ...
    suffix = uuid.uuid4().hex[:8]
    create_resp = await async_client.post(f"/api/v1/workspaces/{ws_id}/invites", json={
        "email": f"revoke_me_{suffix}@test.com",
        "role_id": str(role_id)
    })
