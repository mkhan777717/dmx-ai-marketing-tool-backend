...
async def setup_members_env(async_db: AsyncSession, async_client: AsyncClient):
    ...
    # Setup member
    member_user = User(
        email=f"member3b_{suffix}@test.com", supabase_user_id=uuid.uuid4()
    )
    ...
    for action in ["read", "update", "delete"]:
        result = await async_db.execute(
            select(Permission).filter_by(resource="workspace_member", action=action)
        )
        p = result.scalar_one_or_none()
        if not p:
            p = Permission(
                name=f"workspace_member.{action}",
            ...
            try:
                ...
                result = await async_db.execute(
                    select(Permission).filter_by(
                        resource="workspace_member", action=action
                    )
                )
                p = result.scalar_one_or_none()
        perms.append(p)
...
async def test_change_member_role(
    async_client: AsyncClient, setup_members_env, async_db: AsyncSession
):
    ws_id = setup_members_env["ws"].id
    ...
    response = await async_client.patch(
        ...
        json={"role_id": str(owner_role_id)},
    )
...
async def test_suspend_member(async_client: AsyncClient, setup_members_env):
    ...
    response = await async_client.post(
        f"/api/v1/workspaces/{ws_id}/members/{mem_id}/suspend"
    )
    assert response.status_code == 200
