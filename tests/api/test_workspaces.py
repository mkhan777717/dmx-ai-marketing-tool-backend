...
async def setup_test_auth(async_db: AsyncSession, async_client: AsyncClient):
    ...
    for action in ["update", "delete", "read"]:
        result = await async_db.execute(select(Permission).filter_by(resource="workspace", action=action))
        p = result.scalar_one_or_none()
        if not p:
            p = Permission(name=f"workspace.{action}", resource="workspace", action=action)
            async_db.add(p)
            try:
                await async_db.commit()
    ...
    for role_id, perm_id in [(owner_role.id, perm1.id), (owner_role.id, perm2.id), (owner_role.id, perm3.id), (viewer_role.id, perm3.id)]:
    ...
    return {"user": user, "user2": user2, "owner_role": owner_role, "viewer_role": viewer_role, "suffix": suffix}
...
async def test_create_workspace(async_client: AsyncClient, setup_test_auth):
    ...
    response = await async_client.post("/api/v1/workspaces", json={
        "name": f"New API Workspace {suffix}"
    })
...
async def test_get_workspace_and_permissions(async_client: AsyncClient, setup_test_auth, async_db: AsyncSession):
    ...
    ws = Workspace(name="Test WS", slug=f"test-ws-{suffix}", owner_id=setup_test_auth["user"].id, status=WorkspaceStatus.ACTIVE)
    async_db.add(ws)
    await async_db.commit()
...
async def test_update_workspace(async_client: AsyncClient, setup_test_auth, async_db: AsyncSession):
    suffix = setup_test_auth["suffix"]
    ws = Workspace(name="Update Test WS", slug=f"update-ws-{suffix}", owner_id=setup_test_auth["user"].id)
    async_db.add(ws)
    await async_db.commit()
    await async_db.refresh(ws)
...
async def test_soft_delete_workspace(async_client: AsyncClient, setup_test_auth, async_db: AsyncSession):
    suffix = setup_test_auth["suffix"]
    ws = Workspace(name="Delete Test WS", slug=f"delete-ws-{suffix}", owner_id=setup_test_auth["user"].id)
    async_db.add(ws)
    await async_db.commit()
    await async_db.refresh(ws)
...
async def test_transfer_ownership(async_client: AsyncClient, setup_test_auth, async_db: AsyncSession):
    suffix = setup_test_auth["suffix"]
    ws = Workspace(name="Transfer Test WS", slug=f"transfer-ws-{suffix}", owner_id=setup_test_auth["user"].id)
    async_db.add(ws)
    await async_db.commit()
    await async_db.refresh(ws)
