...
@patch("app.services.campaign_scheduler.AuditLogService.create_audit_log")
async def test_schedule_campaign_new(
    mock_audit_create,
    mock_schedule_repo,
    mock_campaign_id,
    mock_workspace_id,
    ...
    schedule_data = CampaignScheduleCreate(
        publish_date=datetime.now(timezone.utc) + timedelta(minutes=5), timezone="UTC"
    )
