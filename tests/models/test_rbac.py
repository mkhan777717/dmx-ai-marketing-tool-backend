...
from app.repositories.rbac import role_permission_repo, role_repo
...
async def test_permission_uniqueness(async_db: AsyncSession):
    ...
    perm1 = Permission(
        name=f"test_{suffix}.read", resource=f"test_{suffix}", action="read"
    )
    async_db.add(perm1)
    ...
    perm3 = Permission(
        name=f"other_{suffix}.read", resource=f"test_{suffix}", action="read"
    )
    async_db.add(perm3)
    with pytest.raises(IntegrityError):
        await async_db.commit()
...
async def test_role_and_assignment(async_db: AsyncSession):
    ...
    # Create perm
    perm = Permission(
        name=f"module_{suffix}.write", resource=f"module_{suffix}", action="write"
    )
    async_db.add(perm)
    await async_db.commit()
...
async def test_clone_system_role(async_db: AsyncSession):
    ...
    # Setup system role
    sys_role = Role(
        name=f"SysAdmin_{suffix}", role_type=RoleType.SYSTEM, is_system=True
    )
    async_db.add(sys_role)
    perm = Permission(
        name=f"sys_{suffix}.read", resource=f"sys_{suffix}", action="read"
    )
    async_db.add(perm)
    await async_db.commit()
    ...
    ws_id = ws.id
    cloned = await role_repo.clone_system_role_to_workspace(
        async_db, sys_role.id, ws_id, "ClonedAdmin"
    )
