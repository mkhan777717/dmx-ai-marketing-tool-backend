...
async def setup_db(async_db: AsyncSession):
    ...
    ws = Workspace(name="Test WS", slug=f"test-ws-{uuid.uuid4()}", owner_id=owner.id, status=WorkspaceStatus.ACTIVE)
    async_db.add(ws)
    await async_db.commit()
...
async def test_member_lifecycle(async_db: AsyncSession, setup_db):
    user = User(email=f"lifecycle_{uuid.uuid4()}@test.com", supabase_user_id=uuid.uuid4())
    async_db.add(user)
    await async_db.commit()
