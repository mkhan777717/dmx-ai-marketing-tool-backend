...
class WorkspaceInviteRepository(BaseRepository[WorkspaceInvite]):
    async def create_invitation(self, db: AsyncSession, obj_in: dict) -> WorkspaceInvite:
    ...
    async def get_by_token(self, db: AsyncSession, token: str) -> WorkspaceInvite | None:
        stmt = select(self.model).where(self.model.token == token)
        result = await db.execute(stmt)
        return result.scalar_one_or_none()

    async def get_pending_invites(self, db: AsyncSession, workspace_id: uuid.UUID) -> Sequence[WorkspaceInvite]:
    ...
    async def accept_invite(self, db: AsyncSession, invite_id: uuid.UUID) -> WorkspaceInvite | None:
        invite = await self.get_by_id(db, invite_id)
        if invite and invite.status == InviteStatus.PENDING and invite.expires_at > datetime.now(timezone.utc):
    ...
    async def revoke_invite(self, db: AsyncSession, invite_id: uuid.UUID) -> WorkspaceInvite | None:
        invite = await self.get_by_id(db, invite_id)
    ...
    async def resend_invite(self, db: AsyncSession, invite_id: uuid.UUID, new_expires_at: datetime) -> WorkspaceInvite | None:
        invite = await self.get_by_id(db, invite_id)
        if invite and invite.status == InviteStatus.PENDING:
