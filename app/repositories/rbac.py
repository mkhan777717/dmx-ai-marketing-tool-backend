...
class RoleRepository(BaseRepository[Role]):
    async def get_system_roles(self, db: AsyncSession) -> Sequence[Role]:
        stmt = select(self.model).where(self.model.is_system == True).order_by(self.model.name)
        result = await db.execute(stmt)
        return result.scalars().all()

    async def get_workspace_roles(self, db: AsyncSession, workspace_id: uuid.UUID) -> Sequence[Role]:
    ...
    async def clone_system_role_to_workspace(
        self, db: AsyncSession, system_role_id: uuid.UUID, workspace_id: uuid.UUID, new_name: str
    ) -> Role | None:
        # Fetch the system role with its permissions
        stmt = select(self.model).options(selectinload(self.model.role_permissions)).where(
            self.model.id == system_role_id,
...
class RolePermissionRepository(BaseRepository[RolePermission]):
    async def get_role_permissions(self, db: AsyncSession, role_id: uuid.UUID) -> Sequence[Permission]:
        stmt = (
            select(Permission)
    ...
    async def assign_permission(self, db: AsyncSession, role_id: uuid.UUID, permission_id: uuid.UUID) -> RolePermission | None:
    ...
    async def remove_permission(self, db: AsyncSession, role_id: uuid.UUID, permission_id: uuid.UUID) -> bool:
