...
class CampaignContent(Base, TimestampMixin, TenantMixin, AuditMixin, SoftDeleteMixin):
    ...
    status: Mapped[ContentStatus] = mapped_column(
        String, default=ContentStatus.DRAFT, nullable=False
    )
    ...
    parent_version_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("campaign_content.id", ondelete="SET NULL"), nullable=True
    )
    ...
    campaign: Mapped["Campaign"] = relationship(
        "Campaign", back_populates="contents", lazy="selectin"
    )

    parent_version: Mapped["CampaignContent"] = relationship(
        "CampaignContent", remote_side="CampaignContent.id", lazy="selectin"
    )
    assets: Mapped[list["Asset"]] = relationship(
        "Asset", secondary=content_asset_association, lazy="selectin"
    )
