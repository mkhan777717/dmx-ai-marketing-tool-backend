...
campaign_asset_association = Table(
    ...
    Base.metadata,
    Column(
        "campaign_id", ForeignKey("campaigns.id", ondelete="CASCADE"), primary_key=True
    ),
...
class Campaign(Base, TimestampMixin, TenantMixin, AuditMixin, SoftDeleteMixin):
    ...
    status: Mapped[CampaignStatus] = mapped_column(
        String, default=CampaignStatus.DRAFT, nullable=False
    )
    ...
    workspace: Mapped["Workspace"] = relationship(
        "Workspace", lazy="selectin", back_populates="campaigns"
    )
    ...
    assets: Mapped[list["Asset"]] = relationship(
        "Asset", secondary=campaign_asset_association, lazy="selectin"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "ix_campaigns_workspace_name", "workspace_id", "campaign_name", unique=True
        ),
    )
