...
class Plan(Base, TimestampMixin):
    ...
    features: Mapped[dict | list | None] = mapped_column(
        JSONB,
        nullable=True,
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "monthly_price >= 0",
            name="ck_plans_monthly_price",
        ),
        CheckConstraint(
            "yearly_price >= 0",
            name="ck_plans_yearly_price",
        ),
        CheckConstraint(
            "max_users >= 0",
            name="ck_plans_max_users",
        ),
        CheckConstraint(
            "max_workspaces >= 0",
            name="ck_plans_max_workspaces",
        ),
        CheckConstraint(
            "max_ai_credits >= 0",
            name="ck_plans_max_ai_credits",
        ),
    )
