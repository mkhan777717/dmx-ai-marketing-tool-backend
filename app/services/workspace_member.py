...
class WorkspaceMemberService:
    ...
    @staticmethod
    async def _protect_owner_role(
        db: AsyncSession, workspace_id: uuid.UUID, member_to_modify: WorkspaceMember
    ) -> None:
        ...
        if (
            member_to_modify.role_id == owner_role.id
            and member_to_modify.status == MemberStatus.ACTIVE
        ):
            ...
            if len(active_owners) <= 1:
                raise HTTPException(
                    ...
                    detail="Cannot remove or demote the last active owner of the workspace.",
                )
    ...
    async def change_role(
        ...
        if (
            new_role_id == owner_role.id or member.role_id == owner_role.id
        ) and workspace.owner_id != user.id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only the workspace owner can assign or remove the Owner role.",
            )
    ...
    async def suspend_member(
        db: AsyncSession, user: User, workspace: Workspace, member_id: uuid.UUID
    ) -> WorkspaceMember:
        member = await WorkspaceMemberService.get_member(db, workspace.id, member_id)
