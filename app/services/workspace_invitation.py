...
from app.constants.enums import InviteStatus, MemberStatus, NotificationType, NotificationPriority

class WorkspaceInvitationService:
    @staticmethod
    async def get_invites(db: AsyncSession, workspace_id: uuid.UUID) -> Sequence[WorkspaceInvite]:
        return await workspace_invite_repo.get_pending_invites(db, workspace_id)

    @staticmethod
    async def create_invite(db: AsyncSession, user: User, workspace: Workspace, email: str, role_id: uuid.UUID) -> WorkspaceInvite:
        ...
        if existing_user:
            existing_member = await workspace_member_repo.get_member(db, workspace.id, existing_user.id)
        ...
        invite = await workspace_invite_repo.create_invitation(db, {
        ...
        inviter_name = f"{user.first_name or ''} {user.last_name or ''}".strip() or "A member"
        await email_service.send_invitation(email, workspace.name, invite.token, inviter_name)
    ...
    @staticmethod
    async def revoke_invite(db: AsyncSession, user: User, workspace: Workspace, invite_id: uuid.UUID) -> WorkspaceInvite:
    ...
    @staticmethod
    async def resend_invite(db: AsyncSession, user: User, workspace: Workspace, invite_id: uuid.UUID) -> WorkspaceInvite:
        invite = await workspace_invite_repo.get_by_id(db, invite_id)
        if not invite or invite.workspace_id != workspace.id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Invitation not found")
        ...
        invite = await workspace_invite_repo.resend_invite(db, invite.id, new_expires_at)
        
        await email_service.resend_invitation(invite.email, workspace.name, invite.token)
    ...
    @staticmethod
    async def accept_invite(db: AsyncSession, authenticated_user: User, token: str) -> None:
        ...
        if invite.status == InviteStatus.EXPIRED or invite.expires_at < datetime.now(timezone.utc):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invitation has expired")
            
        if invite.email != authenticated_user.email:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Invitation email does not match authenticated user email")
            
        # Prevent duplicate member (in case they already got in through other means)
        existing_member = await workspace_member_repo.get_member(db, invite.workspace_id, authenticated_user.id)
        if existing_member and existing_member.status != MemberStatus.REMOVED:
