...
class WorkspaceService:
    ...
    async def create_workspace(
        db: AsyncSession, user: User, data: WorkspaceCreateInternal
    ) -> Workspace:
    ...
    async def update_workspace(
        db: AsyncSession, user: User, workspace: Workspace, data: WorkspaceUpdate
    ) -> Workspace:
        ...
        if "slug" in update_data and update_data["slug"] != workspace.slug:
            existing = await workspace_repo.get_by_slug(
                db, update_data["slug"], include_deleted=True
            )
            if existing:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
        ...
        updated_workspace = await workspace_repo.update(
            db, db_obj=workspace, obj_in=update_data
        )
    ...
    async def transfer_ownership(
        ...
        new_owner_member = await workspace_member_repo.get_member(
            db, workspace.id, data.new_owner_id
        )
        ...
        await workspace_repo.update(
            db, db_obj=workspace, obj_in={"owner_id": data.new_owner_id}
        )
        ...
        await workspace_member_repo.transfer_ownership(
            db, workspace.id, user.id, data.new_owner_id, data.new_role_id
        )
