...
class CampaignSchedulerService:
    async def schedule_campaign(
        ...
        campaign_state = await db.execute(
            text(
                "SELECT status FROM campaigns WHERE id = :id AND workspace_id = :ws_id"
            ),
            {"id": str(campaign_id), "ws_id": str(workspace_id)},
        )
        ...
        if camp_status in ["COMPLETED", "ARCHIVED"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Cannot schedule a {camp_status} campaign",
            )

        existing = await campaign_schedule_repo.get_by_campaign_id(
            db, campaign_id, workspace_id
        )

        if existing and existing.status == ScheduleStatus.PUBLISHED:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Campaign is already published",
            )
    ...
    async def publish_immediately(
        ...
        schedule = await campaign_schedule_repo.get_by_campaign_id(
            db, campaign_id, workspace_id
        )
        ...
        updated = await campaign_schedule_repo.update(
            db, db_obj=schedule, obj_in={"status": ScheduleStatus.SCHEDULED}
        )
        ...
        background_tasks.add_task(
            self.execute_publish, db, campaign_id, workspace_id, user_id
        )
        return updated
    ...
    async def pause_schedule(
        ...
        if schedule.status != ScheduleStatus.SCHEDULED:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Only SCHEDULED campaigns can be paused",
            )

        updated = await campaign_schedule_repo.update(
            db,
            db_obj=schedule,
            obj_in={"status": ScheduleStatus.PAUSED, "updated_by": user_id},
        )
        await self._log_audit(
            db,
            workspace_id,
            ...
            "PAUSE_SCHEDULE",
            campaign_id,
            old_values={"status": ScheduleStatus.SCHEDULED.value},
            new_values={"status": ScheduleStatus.PAUSED.value},
        )
        return updated
    ...
    async def resume_schedule(
        ...
        updated = await campaign_schedule_repo.update(
            db,
            db_obj=schedule,
            obj_in={"status": ScheduleStatus.SCHEDULED, "updated_by": user_id},
        )
        await self._log_audit(
            db,
            workspace_id,
            ...
            "RESUME_SCHEDULE",
            campaign_id,
            old_values={"status": ScheduleStatus.PAUSED.value},
            new_values={"status": ScheduleStatus.SCHEDULED.value},
        )
        return updated
    ...
    async def cancel_schedule(
        ...
        if schedule.status in [ScheduleStatus.PUBLISHED, ScheduleStatus.CANCELLED]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Cannot cancel a published or already cancelled schedule",
            )

        updated = await campaign_schedule_repo.update(
            db,
            db_obj=schedule,
            obj_in={"status": ScheduleStatus.CANCELLED, "updated_by": user_id},
        )
        await self._log_audit(
            db,
            workspace_id,
            ...
            "CANCEL_SCHEDULE",
            campaign_id,
            old_values={"status": schedule.status.value},
            new_values={"status": ScheduleStatus.CANCELLED.value},
        )
        return updated
    ...
    async def execute_publish(
        ...
        try:
            ...
            if schedule:
                await campaign_schedule_repo.update(
                    db,
                    db_obj=schedule,
                    obj_in={"status": ScheduleStatus.PUBLISHED, "error_message": None},
                )
                await self._log_audit(
                    db, workspace_id, user_id, "PUBLISH_SUCCESS", campaign_id
                )
                await notification_service.notify_campaign_published(
                    campaign_id, user_id, True
                )
            ...
            if schedule:
                await campaign_schedule_repo.update(
                    db,
                    db_obj=schedule,
                    obj_in={"status": ScheduleStatus.FAILED, "error_message": str(e)},
                )
                await self._log_audit(
                    db,
                    workspace_id,
                ...
                await notification_service.notify_campaign_published(
                    campaign_id, user_id, False, str(e)
                )
    ...
    async def get_history(self, db: AsyncSession, campaign_id: uuid.UUID) -> list:
        ...
        stmt = (
            select(AuditLog)
            .where(
                AuditLog.resource_type == "CAMPAIGN_SCHEDULE",
                AuditLog.resource_id == str(campaign_id),
            )
    ...
    async def _get_and_validate_schedule(
        self, db: AsyncSession, campaign_id: uuid.UUID, workspace_id: uuid.UUID
    ) -> CampaignSchedule:
        schedule = await campaign_schedule_repo.get_by_campaign_id(
            db, campaign_id, workspace_id
        )
    ...
    async def _log_audit(
        ...
        def sanitize_dict(d: dict):
            if not d:
                return None
            return {
                k: str(v) if isinstance(v, uuid.UUID) or isinstance(v, datetime) else v
                for k, v in d.items()
            }
