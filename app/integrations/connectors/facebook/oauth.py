...
class FacebookOAuthHandler:
    ...
    async def exchange_code(self, auth_code: str) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise FacebookAuthError(f"Failed to exchange code: {response.text}")

            token_data = response.json()
            short_lived_token = token_data.get("access_token")

            if not short_lived_token:
                raise FacebookAuthError("Access token missing from exchange response.")
    ...
    async def exchange_for_long_lived_token(self, short_lived_token: str) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            ...
            token_data = response.json()
            token_response = FacebookTokenResponse(**token_data)
            ...
            return {
                ...
                # Facebook doesn't typically provide refresh_tokens for standard user access tokens,
                # instead you get long-lived tokens and refresh them by fetching a new one before it expires.
                "refresh_token": None,
